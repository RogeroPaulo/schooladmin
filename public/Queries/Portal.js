...
const QueryEngine={
    ...
    AuthUser: (email, password, handle) => {
		...
		let url = window.location.protocol+"//"+window.location.host+"/api/portal/login"
	...
	teacher: {
		...
		req_suspendedFromClass: (ClassID, UserID, handle, token)=> {
			let header = {
				"entree-verify": "Bearer " + token
			}
			let payload = {
			...
			let url = window.location.protocol+"//"+window.location.host+"/api/teacher/req_suspendedFromClass"
			QueryEngine.postData(url, payload, header)
			.then(pack => {
				QueryEngine.handle_post(pack , ()=>{
					handle(pack);
